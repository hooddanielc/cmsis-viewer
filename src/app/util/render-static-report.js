...
export default ({report, output, use_dist}) => {
  ...
  const hl_worker_src = fs.readFileSync(path.join(dist, 'cmsis-viewer-highlight-worker.js'), 'utf8');
  local_fs_tags.push(render_localfs_script_tag(hl_worker_path, hl_worker_src, false));
  const app_src = fs.readFileSync(path.join(dist, 'cmsis-viewer-browser.js'), 'utf8');
